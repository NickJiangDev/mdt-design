...
const checkIsInstantiation = (target: React.ComponentClass | React.ReactNode) => {
  if (isComponentClass(target)) {
    ...
    return (props: { [key: string]: string }) => <Target {...props} />;
  }
  if (React.isValidElement(target)) {
    return (props: { [key: string]: string }) => React.cloneElement(target, props);
  }
  return () => target;
};
 ...
 * @param {boolean} allowCheck
 */
const authorize = (authority: string, error?: React.ReactNode, allowCheck?: boolean) => {
  /**
  ...
  return function decideAuthority(target: React.ComponentClass | React.ReactNode) {
    const component = CheckPermissions(authority, target, classError || Exception403, allowCheck);
